...
export function PoolSection() {
  ...
  const shareAfter = useMemo(() => {
    ...
    const pct = (Number(formatUnits(parsedUsdc, 18)) / (Number(formatUnits(data.reserveUsdc, 18)) + Number(formatUnits(parsedUsdc, 18)))) * 100
    return pct
  ...
  const needUsdcApproval =
    !data.isDemo && isConnected && parsedUsdc !== undefined && parsedUsdc > 0n &&
    data.allowanceUsdcPair !== undefined && data.allowanceUsdcPair < parsedUsdc
  const needMethApproval =
    !data.isDemo && isConnected && parsedMeth !== undefined && parsedMeth > 0n &&
    data.allowanceMethPair !== undefined && data.allowanceMethPair < parsedMeth
  const needLpApproval =
    !data.isDemo && isConnected && removeLiquidity > 0n &&
    data.allowanceLpPair !== undefined && data.allowanceLpPair < removeLiquidity
  ...
  const doApprove = (token: 'mUSDC' | 'mETH' | 'LP') => {
    const cfg =
      token === 'LP'
        ? { address: ADDRESSES.pair, abi: pairAbi, args: [ADDRESSES.pair, removeLiquidity] as const }
        : {
  ...
  const addReady = parsedUsdc !== undefined && parsedUsdc > 0n && parsedMeth !== undefined && parsedMeth > 0n
    ...
    <div className="grid gap-6 lg:grid-cols-[440px_1fr]">
          ...
          <CardContent>
            <Tabs defaultValue="add">
              <TabsList className="grid w-full grid-cols-2 rounded-xl bg-black/30">
                ...
                <TabsTrigger value="remove" className="rounded-lg data-[state=active]:bg-primary data-[state=active]:text-white">
              ...
              <TabsContent value="add" className="mt-4 space-y-2">
                ...
                <TokenInput
                  ...
                  balance={isConnected || data.isDemo ? formatToken(data.methBalance, 18, 4) : undefined}
                ...
                <div className="space-y-1.5 rounded-2xl border border-white/5 bg-black/20 px-4 py-3 text-xs">
                  ...
                  {shareAfter !== undefined && (
                    <div className="flex justify-between">
                      ...
                      <span className="font-medium text-primary">{formatNumber(shareAfter, 3)}%</span>
                    </div>
                  )}
                </div>

                {needUsdcApproval ? (
                  <Button className="h-12 w-full rounded-2xl font-semibold" disabled={busy} onClick={wrap(() => doApprove('mUSDC'))}>
                    {busy ? 'Approving…' : 'Approve mUSDC'}
                  </Button>
                ) : needMethApproval ? (
                  <Button className="h-12 w-full rounded-2xl font-semibold" disabled={busy} onClick={wrap(() => doApprove('mETH'))}>
                  ...
                  <Button
                    ...
                    onClick={wrap(doAdd)}
                  >
                    {busy ? 'Adding…' : data.isDemo ? 'Add liquidity (demo)' : isConnected ? 'Add liquidity' : 'Connect wallet'}
                  </Button>
                )}
              </TabsContent>
              ...
              <TabsContent value="remove" className="mt-4 space-y-4">
                <div className="rounded-2xl border border-white/5 bg-black/30 p-5 text-center">
                  <div className="text-4xl font-semibold tracking-tight text-primary">{removePct}%</div>
                  ...
                  <div className="mt-2 flex justify-between text-xs text-muted-foreground">
                    {[25, 50, 75, 100].map((p) => (
                      <button key={p} onClick={() => setRemovePct(p)} className="rounded-md px-2 py-1 hover:bg-white/5">
                        {p}%
                      </button>
                    ))}
                  </div>
                ...
                <div className="space-y-1.5 rounded-2xl border border-white/5 bg-black/20 px-4 py-3 text-xs">
                  <div className="flex justify-between">
                    ...
                    <span className="font-medium">{formatNumber(pooledUsdc * (removePct / 100), 2)} mUSDC</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground" />
                    <span className="font-medium">{formatNumber(pooledMeth * (removePct / 100), 5)} mETH</span>
                  </div>
                </div>
                {needLpApproval ? (
                  <Button className="h-12 w-full rounded-2xl font-semibold" disabled={busy} onClick={wrap(() => doApprove('LP'))}>
                  ...
                  >
                    {busy ? 'Removing…' : data.isDemo ? 'Remove liquidity (demo)' : lpBalance === 0n ? 'No LP position' : 'Remove liquidity'}
                  </Button>
                )}
              </TabsContent>
            </Tabs>
      ...
      <div className="space-y-6">
        <div className="grid gap-4 sm:grid-cols-3">
          <StatCard icon={Droplets} label="Pool TVL" value={`$${formatCompact(Number(formatUnits(data.reserveUsdc, 18)) * 2)}`} sub="mUSDC side valued ×2" delay={0.05} />
          <StatCard icon={PieChart} label="Your pool share" value={`${formatNumber(lpShare * 100, 3)}%`} sub={`${formatToken(lpBalance, 18, 2)} LP tokens`} delay={0.1} />
          <StatCard icon={Plus} label="Position value" value={`$${formatCompact(pooledUsdc * 2)}`} sub={lpBalance > 0n ? `${formatNumber(pooledUsdc, 2)} mUSDC + ${formatNumber(pooledMeth, 4)} mETH` : 'No position yet'} delay={0.15} />
        </div>
            ...
            <CardHeader>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <CardDescription>Constant-product AMM · fee 0.30% · LP token BS-LP</CardDescription>
                </div>
              </div>
            </CardHeader>
            <CardContent className="space-y-5">
              {/* Composition bars */}
              <PoolBar label="mUSDC" amount={Number(formatUnits(data.reserveUsdc, 18))} total={Number(formatUnits(data.reserveUsdc, 18)) * 2} color="#2775ca" />
              <PoolBar label="mETH" amount={Number(formatUnits(data.reserveMeth, 18)) * price} total={Number(formatUnits(data.reserveUsdc, 18)) * 2} color="#8a9cff" note={`${formatNumber(Number(formatUnits(data.reserveMeth, 18)), 3)} mETH`} />
              <div className="rounded-2xl border border-white/5 bg-black/20 p-4 text-xs leading-relaxed text-muted-foreground">
                <span className="font-semibold text-foreground">How it works.</span> LPs deposit both sides of the pair and
                receive BS-LP tokens representing their share. Every swap pays a 0.30% fee into the pool, so LP tokens
                become redeemable for a growing amount of underlying over time. Removing liquidity burns BS-LP and
                returns your proportional share of both tokens.
              </div>
            </CardContent>
          </Card>
        </motion.div>
...
function PoolBar({ label, amount, total, color, note }: { label: string; amount: number; total: number; color: string; note?: string }) {
  const pct = total > 0 ? Math.min((amount / total) * 100, 100) : 0
  return (
    <div>
      <div className="mb-1.5 flex justify-between text-xs">
        <span className="font-medium text-muted-foreground">{label}</span>
        <span className="font-medium">{note ?? `$${formatCompact(amount)}`} · {formatNumber(pct, 1)}%</span>
      </div>
